...
interface SaleItemProps {
    ...
    isEdit: boolean
    defaultValues: SaleInvoiceItem
    handleClose: () => void
}

const SaleInvoiceItemForm = ({ setModal, isEdit, defaultValues, handleClose }: SaleItemProps) => {
    const { addSaleInvoiceItem, editSaleInvoiceItem } = useSaleInvoiceItemStore()

    const { register, handleSubmit, watch, reset, formState: { errors } } = useForm<SaleInvoiceItem>({
        defaultValues
    });
    ...
    const onSubmit = async (data: SaleInvoiceItem) => {
        const total = data.price * data.quantity

        if (isEdit) {
            await editSaleInvoiceItem(defaultValues?.id as string, { ...data, total })
            handleClose()
        } else {
            await addSaleInvoiceItem({ ...data, total });
            reset();
            setModal(false);
        }
...
export default SaleInvoiceItemForm